...
const CATEGORIES = ['access', 'logging', 'billing', 'email', 'session', 'edge', 'webhook', 'secrets', 'csrf', 'waf', 'mcp', 'llm', 'consent', 'privacy'];
...
while (reg.protocols.length < TARGET) {
  ...
  const protocolId = `planning/sp65-${svc}-${cat}-${String(i).padStart(3, '0')}`;
  i += 1;
  ...
  reg.protocols.push({
    ...
    mitigationSteps: [
      `Wave65 ${cat} on ${svc}`,
      'Linked via expand-em1-legal-links-wave65',
      'T05 verify + T12 obligation filter',
    ],
    verifyPatternIds: [`SP65-${i}`],
    status: 'active',
  });
}
...
console.log(`✓ Security protocol registry wave 65 — ${reg.protocolCount} protocols`);
