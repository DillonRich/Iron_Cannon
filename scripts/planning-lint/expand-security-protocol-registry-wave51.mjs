#!/usr/bin/env node
/** Grow security-protocol-registry toward 400 active protocols */
...
while (reg.protocols.length < TARGET) {
  ...
  const protocolId = `planning/sp51-${svc}-${cat}-${String(i).padStart(3, '0')}`;
  i += 1;
...
console.log(`✓ Security protocol registry wave 51 — ${reg.protocolCount} protocols`);
