...
const EXTRA = [
  'armor/surface-session',
  registry.protocols.find((p) => p.protocolId.startsWith('planning/sp52-owasp-'))?.protocolId,
  registry.protocols.find((p) => p.protocolId.startsWith('planning/sp52-ironcannon-'))?.protocolId,
...
console.log(`✓ Per-flow scope matrix wave 52 — ${matrix.flows.length} flows enriched`);
