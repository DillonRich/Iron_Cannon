...
const CATEGORIES = ['access', 'logging', 'billing', 'email', 'session', 'edge', 'webhook', 'secrets', 'csrf', 'waf', 'mcp', 'llm', 'xss', 'hsts'];
...
while (reg.protocols.length < TARGET) {
  ...
  const protocolId = `planning/sp66-${svc}-${cat}-${String(i).padStart(3, '0')}`;
  i += 1;
  ...
  reg.protocols.push({
    ...
    mitigationSteps: [
      `Wave66 ${cat} on ${svc}`,
      'Linked via expand-em1-nextjs-owasp-links-wave66',
      'TGS v2 obligation depth floor',
    ],
    verifyPatternIds: [`SP66-${i}`],
    status: 'active',
  });
}
...
console.log(`✓ Security protocol registry wave 66 — ${reg.protocolCount} protocols`);
