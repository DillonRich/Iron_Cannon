...
const CATEGORIES = ['access', 'logging', 'billing', 'email', 'session', 'edge', 'webhook', 'secrets', 'mcp', 'rag', 'agent', 'dispute'];
...
while (reg.protocols.length < TARGET) {
  ...
  const protocolId = `planning/sp67-${svc}-${cat}-${String(i).padStart(3, '0')}`;
  i += 1;
  ...
  reg.protocols.push({
    ...
    mitigationSteps: [
      `Wave67 ${cat} on ${svc}`,
      'Linked via expand-em1-production-links-wave67',
      'Phase 3 production-confidence gate',
    ],
    verifyPatternIds: [`SP67-${i}`],
    status: 'active',
  });
}
...
console.log(`✓ Security protocol registry wave 67 — ${reg.protocolCount} protocols`);
