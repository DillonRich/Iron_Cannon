...
while (reg.protocols.length < TARGET) {
  ...
  const protocolId = `planning/sp53-${svc}-${cat}-${String(i).padStart(3, '0')}`;
  i += 1;
  ...
  reg.protocols.push({
    ...
    category: cat,
    scopeServices: [svc],
    mitigationSteps: [`${cat} on ${svc}`, 'EM-2 matrix', 'T05 proof'],
    verifyPatternIds: [`SP53-${i}`],
    status: 'active',
  });
}
reg.protocolCount = reg.protocols.length;
writeFileSync(path, JSON.stringify(reg, null, 2) + '\n');
console.log(`✓ Security protocols wave 53 — ${reg.protocolCount}`);
