#!/usr/bin/env node
/** Create reference cards for LEG-W53 sourceRefId values (corpus integrity) */
...
for (const ob of ADD.obligations) {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        refId,
        sourceUrl: `https://ironcannon.dev/planning/w53/${slug}`,
        lastVerified: today,
        title: ob.title,
        provider,
        excerpt: `${ob.id}: ${ob.title}`,
        embeddingHint: `${ob.id} ${ob.category} ${ob.detect?.type ?? 'pattern'}`,
        tags: ['planning', 'wave53', 'obligation-source'],
      },
      null,
      2,
    ) + '\n',
