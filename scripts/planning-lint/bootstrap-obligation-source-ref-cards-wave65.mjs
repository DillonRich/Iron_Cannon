...
const ADD = JSON.parse(
  readFileSync(join(ROOT, 'docs/engine/specimens/obligation-wave65-additions.json'), 'utf8'),
);
...
for (const ob of ADD.obligations) {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        refId,
        sourceUrl: `https://ironcannon.dev/planning/W65/${slug}`,
        lastVerified: today,
        title: ob.title,
        provider,
        excerpt: `${ob.id}: ${ob.title}`,
        embeddingHint: `${ob.id} ${ob.category} ${ob.detect?.type ?? 'pattern'}`,
        tags: ['planning', 'wave65', 'obligation-source'],
      },
      null,
      2,
    ) + '\n',
...
console.log(`✓ Obligation source ref cards wave65 — ${created} created`);
