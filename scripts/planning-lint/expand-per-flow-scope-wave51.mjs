...
const EXTRA = [
  ...
  registry.protocols.find((p) => p.protocolId.startsWith('planning/sp51-stripe-'))?.protocolId,
  registry.protocols.find((p) => p.protocolId.startsWith('planning/sp51-legal-'))?.protocolId,
...
console.log(`✓ Per-flow scope matrix wave 51 — ${matrix.flows.length} flows enriched`);
