#!/usr/bin/env node
/** Queue unpublished P0 inventory across all in-scope vendors (quality harvest pass) */
import { readFileSync, writeFileSync, readdirSync, existsSync, mkdirSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
import { PATHS } from './lib/paths.mjs';
import { refIdFromUrl, specimenFilename } from './lib/ref-id.mjs';

const ROOT = join(dirname(fileURLToPath(import.meta.url)), '../..');
const limit = Number(process.argv.find((a) => a.startsWith('--limit='))?.split('=')[1] ?? '250');
const VENDORS = ['stripe', 'cloudflare', 'resend', 'nextjs', 'owasp'];

function loadRules(provider) {
  const p = join(PATHS.priorityRules, `${provider}.json`);
  if (!existsSync(p)) return null;
  return JSON.parse(readFileSync(p, 'utf8'));
}

function scoreLink(link, rules) {
  const path = new URL(link.url).pathname.toLowerCase();
  for (const ex of rules.excludePatterns ?? []) {
    if (path.includes(ex.toLowerCase())) return -1;
  }
  if (/pnpm-lock|package\.json|\.ya?ml$|engineindex\.md|driveindex\.md/i.test(link.url)) return -1;
  for (const inc of rules.includePatterns ?? []) {
    if (path.includes(inc.toLowerCase())) return 2;
  }
  return 1;
}

const existing = new Set();
for (const f of readdirSync(PATHS.references)) {
  if (!f.endsWith('.specimen.json')) continue;
  const c = JSON.parse(readFileSync(join(PATHS.references, f), 'utf8'));
  if (c.refId) existing.add(c.refId);
}

const items = [];
for (const provider of VENDORS) {
  const invPath = join(PATHS.inventory, `${provider}.json`);
  if (!existsSync(invPath)) continue;
  const { links } = JSON.parse(readFileSync(invPath, 'utf8'));
  const rules = loadRules(provider);
  if (!rules) continue;
  for (const link of links) {
    if (/llms\.txt|llms-full|\/samples/i.test(link.url)) continue;
    const refId = refIdFromUrl(link.url, provider);
    if (existing.has(refId)) continue;
    const score = scoreLink(link, rules);
    if (score < 0) continue;
    items.push({
      refId,
      url: link.url,
      title: link.title,
      provider,
      priority: score >= 2 ? 'P0' : 'P1',
      appliesTo: rules.appliesToDefault ?? [],
      specimenFile: specimenFilename(refId),
      score,
    });
  }
}

items.sort((a, b) => b.score - a.score || a.provider.localeCompare(b.provider));
const capped = items.slice(0, limit);

mkdirSync(join(ROOT, 'harvest-data'), { recursive: true });
writeFileSync(
  PATHS.queue,
  JSON.stringify(
    { built: new Date().toISOString(), mode: 'vendor-depth', queued: capped.length, items: capped },
    null,
    2,
  ) + '\n',
  'utf8',
);
console.log(`✓ Vendor depth queue — ${capped.length} URLs (${VENDORS.join(', ')})`);
